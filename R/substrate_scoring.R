multi_candidate_screener <- function(screener_dt, candidates_dt, kinase, 
  ...
  candidate_dt <- data.table(reshape2::melt(setNames(candidate_hits,names), 
                            id.vars = c("kinase", "active", "perf", "score", 
                            "cutpoint")))
...
peptide_screener <- function(screener_dt, candidates_dt, target_kinase, 
                             method = c("prod", "log2_sum", "w_prod"), 
                             pval_corr = FALSE,
                             family = FALSE ){
  ...
  scores_merge <- data.table(merge(scores_quantiles, sub_score[, score, by = kinase], 
                       by = "kinase"))
...
get_score <- function(odds, method = c("prod", "log2_sum", "w_prod"), pval = NULL){
  ...
  if (method == "prod"){
    ...
    pseudo_count = 1
    score <- sum(log(odds/geo_mean(odds)), na.rm = T)
  }else{
    pval_inv <- 1/pval
    log_pval <- log2(pval_inv)/sum(log(pval_inv))
...
fisher_long <- function(fisher_dt, n = NULL){
  if (is.null(n)){
    pval_long <- data.table(reshape2::melt(fisher_dt[[1]], varnames=c(
      "amino_acid", "flank_pos"), value.name="fisher_pval")
    )
    odds_long <- data.table(reshape2::melt(fisher_dt[[2]], varnames=c(
      "amino_acid", "flank_pos"), value.name="fisher_odds")
    )
    merge_dt <- merge(odds_long, pval_long, by = c("amino_acid", "flank_pos")) 
  }else{
    merge_dt <- data.table(reshape2::melt(fisher_dt, varnames=c("amino_acid", 
                          "flank_pos"), value.name="fisher_odds"))
  }
  
  merge_dt[,barcode:=paste0(amino_acid, ":", flank_pos)]
  merge_dt[, fisher_odds:= ifelse(fisher_odds == 0 & fisher_pval > 0.05, NA,
                                  fisher_odds)]
